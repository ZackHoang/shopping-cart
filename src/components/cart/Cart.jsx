            ...
            {cart.length > 0 && 
                cart.map((item) => {
                    return <div key={item.id}>
                        ...
                        <div>
                            <h3>${item.price}</h3>
                            <input type="number" value={item.amount} onChange={(event) => handleChangeAmount(item, event.target.value)}></input>
                            <TiDeleteOutline />
                        </div>
                    </div>
                })
